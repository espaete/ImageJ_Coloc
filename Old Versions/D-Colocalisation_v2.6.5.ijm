...
function recursiveThresholding(staining){
	...
	while (true) {
		...
		run("Analyze Particles...", "size=" + minSize + "-" + maxSize + " circularity=" + circularity + "-1.0 show=Masks");
		run("Make Binary");
		...
		selectWindow("Mask of ThresholdMask");
		run("Create Selection");
		
		close("Mask of ThresholdMask");
		

		selectWindow("OversizedMask");
		run("Make Binary");
		run("Restore Selection");
		run("Make Inverse");
