...
#@ Boolean (label = "Despeckle?", style="Checkbox", value = true) despeckle1
#@ Boolean (label = "Median + Mean?", style="Checkbox", value = true) medianMean1
...
#@ Boolean (label = "Despeckle?", style="Checkbox", value = true) despeckle2
#@ Boolean (label = "Median + Mean?", style="Checkbox", value = true) medianMean2
...
#@ Boolean (label = "Despeckle?", style="Checkbox", value = true) despeckle3
#@ Boolean (label = "Median + Mean?", style="Checkbox", value = true) medianMean3
...
despeckleList = toString(despeckle1) + "," + toString(despeckle1) + "," + toString(despeckle3);
despeckleList= split(despeckleList,",");
medianMeanList = toString(medianMean1) + "," + toString(medianMean2) + "," + toString(medianMean3);
medianMeanList= split(medianMeanList,",");
...
function quantifyLabeling(staining, recursive, fillHoles, isDilate, isClose, minSize, maxSize, circularity, segment, despeckle, medianMean){
...
function recursiveThresholding(staining){
	ogImage = staining + "_" + areaName + "_raw";
	noBG = staining + "_" + areaName + "_noBG";
	close("C3-Composite");
	getThreshold(lower, upper);
	...
	run("16-bit");
	run("Invert");
	run("Merge Channels...", "c1=" + ogImage + " c2=" + noBG + " c3=Outlines create keep");
...
for (i = 0; i < allStainings.length; i++) {
	quantifyLabeling(allStainings[i], recursiveList[i], fillHolesList[i], isDilateList[i], isCloseList[i], minSizeList[i], maxSizeList[i], circularityList[i], segmentList[i], despeckleList[i], medianMeanList[i]);
}
