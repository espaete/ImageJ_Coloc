	...
	if(stainings.length > 1){
		...
		if(isOpen("Mask-" + stainings[1] + "+" + stainings[0])){
			...
			if(isOpen(stainings[0] + "_" + areaName + "_raw") && isOpen(stainings[1] + "_" + areaName + "_raw")){
				run("Duplicate...", "title=Outlines-" + stainings[0] + "+" + stainings[1]);
				run("Outline");
				run("16-bit");
		...
		rename(mask);
		run("Duplicate...", "title=Outlines " + stainingString);
...
for (i = 0; i < allStainings.length-1; i++) {
	...
	run("Select None");
	run("Make Binary");
	run("Duplicate...","title=Outlines-" + allStainings[i] + "+" + allStainings[i+1]);
	run("Outline");
	run("16-bit");
}
...
if (allStainings.length > 2) {
	...
	run("Select None");
	run("Make Binary");
	run("Duplicate...","title=Outlines-" + allStainings[0] + "+" + allStainings[2]);
	run("Outline");
	run("16-bit");
