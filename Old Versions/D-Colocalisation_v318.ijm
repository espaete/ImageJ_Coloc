...
function removeBackground(staining, channelNr, rbSubtraction, subtractTimes){
	...
	if(staining != ""){
		...
		if (subtractTimes && File.exists(medianFilter + File.separator + "Filtered_" + staining + "-" + filename)){
...
function recursiveThresholding(staining){	
	...
	while (true) {
		...
		run("Invert");
		//run("16-bit");
		imageCalculator("Add", "Output", "Mask of ThresholdMask");
		...
		imageCalculator("AND create", "OversizedMask", "Image");
		rename("OversizedOG");
		run("Invert");
		if (!isOpen("Oversized")){
...
if (oldRoi && File.isDirectory(cutAreasROI)) {
	roiManager("reset");
...
while (doColoc){
	...
	//Enlarge selection of stainng A according to user input
	run("Restore Selection");
