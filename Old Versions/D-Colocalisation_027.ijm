...
function segmentParticles(image){
	//Segments particles via a combination of watershed and dilate and recursice thresholding.
	//Uses the user set maximum particle size to identify oversized particles.
	//Particles of correct size are kept while oversized particles are analyzed with a reduced threshold.
	//Reduces the threshold each time until all oversized particles are gone.
	selectWindow(staining + "_noBG");
	run("Watershed");
	run("Analyze Particles...", "size=" + maxSize + "-Infinity show=Masks");
