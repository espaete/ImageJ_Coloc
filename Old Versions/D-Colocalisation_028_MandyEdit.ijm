...
function removeBackground(staining, rbRadius, filteredImage){
	...
	if(staining != ""){
		...
		//Weird thing
		singleList = getFileList(singleChannels);
		for (i = 0; i < singleList.length; i++) {
			if (singleList[i] == staining + "-" + filename) {
				open(singleChannels + File.separator + singleList[i]);
			}
		}
...
function recursiveThresholding(staining){
	...
	waitForUser("Check for accuracy and modify accordingly ('f' to add, 'backspace' to delete)");
	setBackgroundColor(0,0,0);
	run("Split Channels");
	close("C1-Composite");
	close("C2-Composite");
