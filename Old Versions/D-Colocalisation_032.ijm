...
function getPositiveCells(stainingA, stainingB, minSize, maxSize, circularity, sensitivity, segment, colocCells){
	...
	if(colocCells && sensitivity != "" &&  matches(stainingA, "\\w+.*")&& matches(stainingB, "\\w+.*")){
		...
		while (repeatLoop) {
			...
			close("Outlines_" + stainingA + "+" + stainingB);
			...
			run("8-bit");
			sensitivity = getString("Sensitivity=", "2");
			...
			rename("Mask");
			run("Make Binary");
			segment = getBoolean("Segment?");
			if(segment){
				run("Watershed");
			}else{
				run("Close-");
			}
			run("Duplicate...", "title=Outlines_" + stainingA + "+" + stainingB);
			run("Outline");
			run("16-bit");
			run("Merge Channels...", "c1=" + stainingA + "_" + areaName + "_raw c2=" + stainingB + "_" + areaName + "_raw c3=Outlines_" + stainingA + "+" + stainingB + " create keep");
...
while (repeatLoop) {
	if(allStainings.length == 3){
		...
		run("Outline");
		run("16-bit");
		run("Merge Channels...", "c1=" + staining1 + "_" + areaName + "_raw c2=" + staining2 + "_" + areaName + "_raw c3=" + staining3 + "_" + areaName + "_raw" + " c4=Outlines-" + staining1 + "+" + staining2 + " c5=Outlines-" + staining1 + "+" + staining3 + " c6=Outlines-" + staining3 +  " c7=TripleMask create keep");
		rename("TripleColoc");
